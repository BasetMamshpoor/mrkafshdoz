...
import CategoryFilter from './CafegoryFilter';
import { Switch } from "@heroui/react";
import { useRouter } from 'next/router';

const Filters = ({ setIsOpen, isWholeSale, isAll }) => {
    const router = useRouter()
    const [filters, setFilters] = useState({})
    const [data] = useGetRequest(`/products${isAll ? '/all' : isWholeSale ? '/major' : ''}/getfilters`)
        ...
        <>
            {!!data ? <div className={style.filters}>
                ...
                {!isWholeSale && <>
                    ...
                    <div className={style.yqgi2}>
                        <div className={style.typeF}>
                            <input className={style.checkprice2} type="checkbox" id="price" hidden />
                            ...
                            <div className={style.CrWx}>
                                <div className={style.kFiU}>
                                    <div className={style.vGex}>
                                        <label>از</label>
                                        <Input type="number" name='min' value={filters.min ? filters.min[0].name : data.price.min} min={data.price.min} result={changeUrl} />
                                    </div>
                                    <div className={style.vGex}>
                                        <label>تا</label>
                                        <Input type="number" name='max' value={filters.max ? filters.max[0].name : data.price.max} max={data.price.max} result={changeUrl} />
                                    </div>
                                </div>
                            </div>
                        </div>
