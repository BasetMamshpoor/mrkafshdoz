...
const Majors = ({ setOnOrder, status }) => {
    const { replace, query } = useRouter();
    const { code } = query

    const [currentPage, setCurrentPage] = useState(1)
    const [orders, setOrders, reload, pagination] = useGetPrivateRequest('/user-product-details', currentPage, { code, status })
                ...
                <div className='mb-3' dir="rtl">
                    {!!orders.length > 0 ? <div className='px-1 flex flex-col items-stretch overflow-x-auto overflow-y-hidden'>
                        <ul className='flex items-center sm:px-4 flex-nowrap'>
                            <li className='text-sm font-semibold flex-[0_0_120px] text-center mx-1.5 text-slate-500'>شماره تماس</li>
                            ...
                            <li className='text-sm font-semibold flex-[0_0_220px] text-center mx-1.5 text-slate-500'>اسم محصول</li>
                                ...
                                return (
                                    <li key={o.id} className='my-3 w-full'>
                                        <ul className='flex flex-nowrap w-full items-center rounded-lg shadow-md py-3.5 sm:px-4 bg-gray-50 cursor-pointer duration-300 hover:shadow-lg [&>*]:mx-1.5 [&>*]:text-center' onClick={() => detailOrder(o)}>
                                            <li className='!flex-[0_0_120px] text-[12px]'><a href={`tel:${o.mobile}`}>{o.mobile}</a></li>
                                            <li className='text-sm !flex-[0_0_220px] line-clamp-1 centerOfParent gap-1'><span>{o.user_name}</span></li>
                                            <li className='text-[12px] !flex-[0_0_120px]'>{new Date(o.created_at).toLocaleDateString('fa-IR', { year: 'numeric', month: 'long', day: 'numeric' })}</li>
                                            <li className='text-[12px] !flex-[0_0_220px]'>{o.product_name}</li>
