...
const Majors = ({setOnOrder, status}) => {
    ...
    const [orders, setOrders, reload, pagination, , loading] = useGetPrivateRequest('/user-product-details', currentPage, {
        code,
        status
    })
    ...
    return (
        <>
            {!loading ?
                    ...
                    {!!orders?.length > 0 ?
                        <div className='px-1 flex flex-col items-stretch overflow-x-auto overflow-y-hidden'>
                            <ul className='flex items-center sm:px-4 flex-nowrap'>
                                <li className='text-sm font-semibold flex-[0_0_120px] text-center mx-1.5 text-slate-500'>شماره
                                    تماس
                                </li>
                                ...
                                <li className='text-sm font-semibold flex-[0_0_220px] text-center mx-1.5 text-slate-500'>اسم
                                    محصول
                                </li>
                                    ...
                                    return (
                                        <li key={o.id} className='my-3 w-full'>
                                            <ul className='flex flex-nowrap w-full items-center rounded-lg shadow-md py-3.5 sm:px-4 bg-gray-50 cursor-pointer duration-300 hover:shadow-lg [&>*]:mx-1.5 [&>*]:text-center'
                                                ...
                                                <li className='!flex-[0_0_120px] text-[12px]'><a
                                                    href={`tel:${o.mobile}`}>{o.mobile}</a></li>
                                                <li className='text-sm !flex-[0_0_220px] line-clamp-1 centerOfParent gap-1'>
                                                    <span>{o.user_name}</span></li>
                ...
                : <div className='w-full centerOfParent mt-6'>درحال بارگزاری ...</div>
            }
        </>
    );
};
