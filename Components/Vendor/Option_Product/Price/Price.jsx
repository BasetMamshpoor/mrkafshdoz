...
const Price = ({ setProduct, is_major, offPrice, price, touch, errors, discountTime }) => {

    const input = useRef()
    const newDate = new Date()
    const [checkbox, setCheckbox] = useState()
    const [major, setMajor] = useState(is_major)
    ...
    const getToday = useCallback((date) => (new DateObject(date).toDate().toLocaleDateString('fa-IR').split('/')[2]), [])

    return (
        <>
            <div className="flex md:flex-row flex-col md:items-center gap-4 w-full justify-between">
                <Checkbox
                    size='lg'
                    onValueChange={e => {
                        setMajor(e)
                        setProduct(prev => {
                            // if (e) {
                            //     const { price, offPrice, off_date_to, off_date_from, ...other } = prev
                            //     return { ...other, is_major: e }
                            // }
                            // else
                            return { ...prev, is_major: e ? 1 : 0 }
                        })
                ...
                <div className={style.nJe_3zq_plf}>
                    <Input disabled={major} isNumber={true} type='text' placeholder="قیمت" name='price' result={handleResult} className={style.input} value={price} />
                    {touch.price && errors.price && <span className={style.errors_input}>{errors.price}</span>}
                </div>
                <div className={style.FzwYa_4n4}>
                    <input type="checkbox" name="isOff_uAdd" id={style.IcE_22} hidden onChange={handleCheck} checked={major ? false : checkbox} />
