    ...
    const handleSearch = async event => {
        ...
        await axios.get(`/products/show/${value}`)
            .then(res => {
                ...
                const { brand, category, image, images, created_at, id, offPercent, rate, slug, is_major, ...prev } = cleanState
                let newProductSatet = { ...prev, brand_id: brand.id, category_id: category?.id, images: [], deletingImages: [], deletingColors: [], _method: "PUT", is_major: is_major ? 1 : 0 };
            ...
            <div className={`${style.Kce_1W2M4_6} ${!!id ? style.editing : ''}`} >
                    ...
                    {!id && <div div className={style.nav_search}>
                        <form onSubmit={handleSearch}>
                            <div className={style.search_input}>
                                <input className={style.input} type="number" placeholder='آیدی محصول' defaultValue={query.id} />
                            </div>
                ...
                {!!state ? <div className={style.uTyc_3Waxd1}>
                    <form className={style.WzlProductAdd_tce} onSubmit={handleSubmit}>
                        ...
                        <div className={style.DxwzE_Os_T3}>
                            <Price setProduct={setProduct} is_major={product.is_major} offPrice={product.offPrice} price={product.price} touch={touch} errors={errors} discountTime={{ off_date_from: new Date(product.off_date_from), off_date_to: new Date(product.off_date_to) }} />
                        </div>
