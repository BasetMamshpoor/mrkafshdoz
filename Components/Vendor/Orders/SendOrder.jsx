...
const SendOrder = ({ order_id, setOnOrder }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        await AxiosPrivate.patch(`/order/update/${order_id}`, { order_status_id: 3, postal_reference: data })
            .then(res => { alert(res.data.message || 'سفارش ارسال شد'); OrderList() })
            .catch(err => alert(err.response?.data.message || 'سفارش ارسال نشد'))
            .finally(() => setLoading(false))
    }

    return (
        <>
            <button type='button' onClick={onOpen} className='rounded-md !bg-blue-500 text-white py-2 px-3 gap-4 text-sm'><BsTruck />ارسال سفارش</button>

            <Modal isOpen={isOpen} size="xl" scrollBehavior={'inside'} onOpenChange={onOpenChange}>
                <ModalContent>
                    {(onClose) => (
                        <>
                                ...
                                <form className="flex flex-col items-center" dir="rtl" onSubmit={handleSubmit}>
                                    <div className="relative pb-6 mb-4 min-w-28 max-w-[100%] w-full">
                                        <div className="relative">
                                            ...
                                            <label htmlFor="outlined_1" className="absolute rounded-lg text-sm duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 start-1 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto">روش ارسال</label>
                                        </div>
                                    </div>
                                    <div className="relative pb-6 mb-4 min-w-28 max-w-[100%] w-full">
                                        <div className="relative">
                                            <input required type="text" id="outlined_2" name='code' value={data.code} onChange={handleChange}
                                                className="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 appearance-none dark:text-white focus:outline-none focus:ring-0 peer" placeholder="" />
                                            <label htmlFor="outlined_2" className="absolute rounded-lg text-sm duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 start-1 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto">کد پیگیری ارسال</label>
                                        </div>
                                    </div>
