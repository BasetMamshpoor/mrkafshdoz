...
const Products = ({ total_Items }) => {
    ...
    const [products, setProducts, reload, pagination, setPagination] = useGetRequest(`/products/major${category ? `/filter/${category}` : ''}`, 1, parsedQuery)
    ...
    const loadMoreItems = async (page) => {
        const products = await getAllProducts(query, page, true)
        if (!!products) {
            setProducts(prev => {
